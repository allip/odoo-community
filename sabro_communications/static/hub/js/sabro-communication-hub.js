...
Encoretel Proprietary License v1.0

This software and associated files (the "Software") may only be used (executed, modified, executed after modifications) if you have purchased a valid license from the authors, typically via https://www.allip.co.uk, or if you have received a written agreement from the All-IP Cloud.
...
Copyright (C) 2015-2018  All-IP Cloud <info@allip.co.uk>
...
var centrex_default_line = 'webrtc';
var mediaStream;
var auth;
var confd;
var dird;
...
loader = loadingOverlay.activate();
...
$.ajax({
    ...
    'success': function(result) {
    	...
    	wazo_user_email = result['wazo_user_email'];
    	server = result['wazo_webrtc_url'];
    	port = result['wazo_webrtc_port'];
        wazo_webrtc_url = server;
        wazo_webrtc_port = port;
        ctxStorage.store('server', server);
        ctxStorage.store('port', port);
		username = result['wazo_webrtc_username'];  
		password = result['wazo_webrtc_password'];
        auth = new XiVOAuth({
          host: server,
          https_port: port,
    ...
    'error': function() {
		toastr_alert("error", "You have been logged out!");
    	$("#success_msg_model").html("Error Occur due to server not responding")
    	loadingOverlay.cancel(loader);
		//location.reload();
    	alert("Your session has expired. Please login to SaBRO.");
    	document.location.href="/web/login?redirect=/sabro_communication_hub/static/hub/index.html";
    }
});
    ...
    session.on('failed', function(response, cause) {
        ...
        switch (cause) {
            ...
            case SIP.C.causes.NOT_FOUND:
            	toastr_alert("error", "Sorry, it's not possible to call this number...");
                break;
            case SIP.C.causes.REJECTED:
            	toastr_alert("error", "Call rejected...");
                break;
            case SIP.C.causes.UNAVAILABLE:
            	toastr_alert("error", "There is an error: " + response.reason_phrase);
                break;
            case SIP.C.causes.INCOMPATIBLE_SDP:
            	toastr_alert("error", "There is an error: " + response.reason_phrase);
                break;
            case SIP.C.causes.AUTHENTICATION_ERROR:
            	toastr_alert("error", "Authentification failed");
                break;
            case SIP.C.causes.BUSY:
            	toastr_alert("error", "Busy...");
                break;
...
function loadLines(data) {
    ...
    if (data.lines) {
        if (data.lines.length > 1) {
			...
			webrtc_line_id = false;
            $.each(data.lines, function(key, value) {
				line_type_check = data.lines[key]['endpoint_sip']['username'].slice(-6)
				if (line_type_check.indexOf("w") >= 0){
					webrtc_line_id = data.lines[key]['id'];
				}
                l = parseInt(key);
                l += 1;
                lines.push({
                    text: $.t('app.box.lines.line') +' ' + l + " (" + data.lines[key]['endpoint_sip']['username'] + ")",
                    value: value.id
                });
            });
			if (webrtc_line_id && centrex_default_line == 'webrtc'){
				configureLine(webrtc_line_id); //Auto Select WebRTC Line for Hub
			}
			else{	
				bootbox.prompt({
...
var signInError = function(data) {
    ...
    if (data.status == 0) {
        printDebug("Please check your SaBRO server access!");
        port = auth.port;
        if (USING_ONLY_HTTPS) {
...
function eventLoginConfiguration() {
    var links = [
     ...
     $('[data-link-target=configuration]').on('mousedown', function() {
         configuration();
     });
}
...
var addContact = function() {
    $('#post-contact').on('click', function(e) {
        ...
        if (firstname && $.isNumeric(phonenumber)) {
        	...
        	toastr_alert("error", "There is some problem to add your user, please review your form!");
        }
    });
}
  ...
  if (value.xivo_uuid && value.user_uuid) {
    getUserUUIDPresence(value.xivo_uuid, value.user_uuid)
      .done(function(data) {
        ...
        printDebug("Your SaBRO don't support all features, please upgrade it to the latest version!");
        $('#presenceuser-' + value.xivo_uuid + '-' + value.user_uuid)
...
function loadUserForwards(data) {
    if (data.forwards) {
    	...
    	toastr_alert("error", "Please upgrade your server to the latest SaBRO version!");
...
function loadUserGravatar(email) {
    if (email) {
        ...
        img = "/web/image?model=res.users&field=image_small&id="+uid.toString();
        $('.image-profile').attr('src', img);
    }
}
    ...
    create_personal: function(contact) {
        dird.create_personal_contact(contact)
          .done(function() {
            ...
            lastname = $('#lastname').val('');
            phonenumber = $('#phonenumber').val('');
            toastr_alert("info", "Contact has been added");
          })
          .fail(function(data) {
            printDebug(data);
            toastr_alert("error", "ERROR: Contact has not been added");
          });
    },
...
function toastr_alert(log_type, message){
	toastr.options = {
			  "closeButton": false,
			  "debug": false,
			  "newestOnTop": false,
	...
	Command: toastr[log_type](message)
}
//function getUserPreferences()
//{
//    $.ajax({
//        'type': "POST",
//        'url': '/comm/hub/get/user/preference',
//        'dataType': "json",
//        'success': function(response) {
//        	if (response.centrex_default_line){
//        		centrex_default_line = response.centrex_default_line;
//        	}
//        	printDebug('************User Preference***************');
//        	printDebug(response);
//	} 
//    });	
//}
