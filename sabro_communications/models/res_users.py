...
class ResUsers(models.Model):
    ...
    def create(self, vals):
        if 'sabro_enabled' in list(vals.keys()):
            if vals['sabro_enabled']:
                if not (vals['sabro_user'] if 'sabro_user' in list(vals.keys()) else False):
                    name = vals['name'] if 'name' in list(vals.keys()) else False
                    login = vals['login'] if 'login' in list(vals.keys()) else False
    ...
    def write(self, vals):
        if 'sabro_enabled' in list(vals.keys()):
            if vals['sabro_enabled']:
                sabro_user = vals['sabro_user'] if 'sabro_user' in list(vals.keys()) else self.sabro_user
                if not sabro_user:
                    name = vals['name'] if 'name' in list(vals.keys()) else self.name
                    login = vals['login'] if 'login' in list(vals.keys()) else self.login
                    if '@' not in str(login) or not login or not name:
                        raise ValidationError(_("Please make sure that Email/Login for the user is valid"))
                    RPCobject= self.env['sabro_auth'].sudo().getNodeAuthentication()
